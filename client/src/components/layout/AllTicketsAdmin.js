...
class AllTicketsAdmin extends React.Component {
    constructor(props, context) {
      super(props, context);
      this.state = {
        books: [],
        filter: "All",      //filter for state
    ...
    componentDidMount() {
      M.AutoInit();
      axios.get('/api/tickets/admin/all')
        .then(res => {
            ...
            <table class="highlight responsive-table grey lighten-4 z-depth-2">
              ...
              <tbody>
                {this.state.books.filter((book) => {
                  ...
                  <tr>
                    <td><Link to={`/admin/dashboard/show/${book._id}`}>{book._id}</Link></td>
                    <td>{book.title}</td>
