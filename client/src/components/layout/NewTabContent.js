...
class NewTabContent extends React.Component {
    constructor(props, context) {
      ...
      this.state = {
        ...
        searchFilter: 'Category', //which option the search searches for
   };
  ...
  owner(book) {
    if (book.owner) {
      if (this.state.searchFilter === "Title"){
        return book.title;
  ...
  admin(book) {
    if (book.admin){
    return <Link to = {`/admin/profile/${book.admin._id}`}>{book.admin.name}</Link>
    } else {
      return <p>No Admin</p>;
    }
    ...
    render() {
      return (
        ...
        <div class="panel panel-default">
          ...
          <div class = "row">
            <h4 style = {{margin: 25}}><Link to="/user/newTickets/create"><span class="glyphicon glyphicon-plus-sign" aria-hidden="true"></span> Add Ticket</Link></h4>
          <div/>
          <div class ='row'>
          <button class = {this.Class('All')} Name = 'All' onClick={this.OnClickStatus}>
                            All
                        </button>
                        <button class = {this.Class('New')} Name = 'New' onClick={this.OnClickStatus}>
                            New
                        </button>
                        <button class = {this.Class('In Progress')} Name = 'In Progress' onClick={this.OnClickStatus}>
                            In Progress
                        </button>
                        <button class = {this.Class('Done')} Name = 'Done' onClick={this.OnClickStatus}>
                            Done
                        </button>
          </div>
          <div class = "row">
            ...
            <table class="highlight responsive-table grey lighten-4 z-depth-2">
              ...
              <tbody>
                {this.state.books.filter((book) => {
                  ...
                  <tr>
                    <td><Link to={`/user/newTickets/show/${book._id}`}>{book._id}</Link></td>
                    <td>{book.title}</td>
                    <td>{this.admin(book)}</td>
